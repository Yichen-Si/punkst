...
int32_t cmdManipulateTiles(int32_t argc, char** argv) {
    ...
    pl.add_option("print-index", "Print the index entries to stdout", printIndex)
      ...
      .add_option("n-gene-bins", "Positive maximum gene-bin count for adaptive packing; zero disables TSV-derived gene-bin packaging", mltOptions.n_gene_bins)
      .add_option("gene-bin-mode", "Gene-bin packing mode: adaptive or fixed", mltOptions.gene_bin_mode)
      .add_option("gene-bin-target-molecules", "Target molecules per adaptive gene bin", mltOptions.gene_bin_target_molecules)
      .add_option("gene-bin-singleton-ratio", "Adaptive singleton threshold as a multiple of target molecules", mltOptions.gene_bin_singleton_ratio)
