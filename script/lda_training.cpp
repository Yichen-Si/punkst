...
public:
    ...
    int32_t trainOnline(LatentDirichletAllocation& lda, const std::string& inFile, int32_t batchSize = 128) {
        ntot = 0;
...
private:
    ...
    bool readMinibatch(std::ifstream& inFileStream) {
        ...
        while (nlocal < batchSize) {
            ...
            Document doc;
            if (!reader.parseLine(doc, line, layer)) {
    ...
    bool readMinibatch(std::ifstream& inFileStream, std::vector<std::string>& idens) {
        ...
        while (nlocal < batchSize) {
            ...
            Document doc;
            int32_t x, y;
            if (!reader.parseLine(doc, x, y, line, layer)) {
...
int32_t cmdLDA4Hex(int argc, char** argv) {
    ...
    LatentDirichletAllocation lda(nTopics, nFeatures, nThreads, seed,
        alpha, eta, maxIter, mDelta, kappa, tau0, lda4hex.nUnits() * nEpochs);

    for (int epoch = 0; epoch < nEpochs; ++epoch) {
        int32_t n = lda4hex.trainOnline(lda, inFile, batchSize);
