...
struct JointTransformBatch {
    std::vector<Document> docs;
    std::vector<std::string> ids;
    void clear() { docs.clear(); ids.clear(); }
...
void applyWeights(std::vector<Document>& docs, GammaPoissonJointC4Hex& gp) {
    for (auto& doc : docs) {
        gp.applyWeights(doc);
    }
}

class JointTransformBatchProcessor {
public:
    JointTransformBatchProcessor(GammaPoissonJointC4Hex& gp_,
        std::ostream& results_, std::ostream& clusters_, RowMajorMatrixXd& pseudobulk_)
        : gp(gp_), results(results_), clusters(clusters_), pseudobulk(pseudobulk_),
          M(gp_.nFeatures()), K(gp_.getNumTopics()), B(gp_.getNumClusters()) {}

    void process(JointTransformBatch& batch) {
        if (batch.empty()) return;
        RowMajorMatrixXd doc_topic, doc_cluster;
        gp.do_transform_both(DocumentView(batch.docs), doc_topic, doc_cluster);
        writeTopicRows(batch.ids, doc_topic);
        writeClusterRows(batch.ids, doc_cluster);
...
private:
    void writeTopicRows(const std::vector<std::string>& ids,
        ...
        for (size_t i = 0; i < ids.size(); ++i) {
            if (!ids[i].empty()) results << ids[i] << "\t";
            results << doc_topic(i, 0);
            for (int32_t k = 1; k < K; ++k) results << "\t" << doc_topic(i, k);
            results << "\n";
        }
    }

    void writeClusterRows(const std::vector<std::string>& ids,
        const RowMajorMatrixXd& doc_cluster) {
        for (size_t i = 0; i < ids.size(); ++i) {
            if (!ids[i].empty()) clusters << ids[i] << "\t";
            int32_t best = 0;
            for (int32_t c = 1; c < B; ++c) {
                if (doc_cluster(i, c) > doc_cluster(i, best)) best = c;
            }
            clusters << best;
            for (int32_t c = 0; c < B; ++c) clusters << "\t" << doc_cluster(i, c);
            clusters << "\n";
        }
    }

    GammaPoissonJointC4Hex& gp;
    std::ostream& results;
    std::ostream& clusters;
...
int32_t cmdGammaPoisJointTransform(int argc, char** argv) {
    ...
    pl.add_option("in-data", "Input hex file", inFile)
      ...
      .add_option("in-state", "Input joint Gamma-Poisson state file", stateFile, true)
    ...
    const std::vector<std::string> modelFeatures =
        GammaPoissonTopicJointC::read_state_feature_names(stateFile);
    HexReader reader;
    ...
    GammaPoissonJointC4Hex gp(reader, modal, verbose);
    ...
    const std::string clustersPath = outPrefix + ".clusters.tsv";
    std::ofstream results(resultsPath);
    std::ofstream clusters(clustersPath);
    if (!results) error("Error opening output file: %s for writing", resultsPath.c_str());
    if (!clusters) error("Error opening output file: %s for writing", clustersPath.c_str());
    writeUnitIdHeader(results, use_10x, info_header);
    gp.writeUnitHeader(results);
    writeUnitIdHeader(clusters, use_10x, info_header);
    std::vector<std::string> clusterCols;
    gp.getClusterHeaderCols(clusterCols);
    clusters << clusterCols[0];
    for (size_t i = 1; i < clusterCols.size(); ++i) clusters << "\t" << clusterCols[i];
    clusters << "\n";
    results << std::fixed << std::setprecision(4);
    clusters << std::fixed << std::setprecision(4);

    JointTransformBatchProcessor processor(gp, results, clusters, pseudobulk);
    bool fileopen = true;
    int32_t processed = 0;
    ...
    JointTransformBatch batch;

    if (use_10x) {
        DGEReader10X& dge = *dge_ptr;
        int32_t n_overlap = dge.setFeatureIndexRemap(modelFeatures, false);
        if (n_overlap == 0) error("No overlapping features found between 10X input and model state");
        ...
        if (sorted_by_barcode) {
            while (fileopen && processed < maxUnits) {
                ...
                fileopen = dge.readMinibatch(batch.docs, barcode_idx,
                    batchSize, maxUnits - processed, minCountInt);
            ...
            while (cursor < all_docs.size() && processed < maxUnits) {
                ...
                take = std::min(take, static_cast<size_t>(maxUnits - processed));
        ...
        while (fileopen && processed < maxUnits) {
            ...
            fileopen = gp.readMinibatch(inFileStream, batch.docs, batch.ids,
                batchSize, minCountInt, maxUnits - processed);
            if (batch.empty()) break;
            processor.process(batch);
            processed += static_cast<int32_t>(batch.size());
        }
    ...
    notice("Cluster results written to %s", clustersPath.c_str());

    const std::string pseudobulkPath = outPrefix + ".pseudobulk.tsv";
    std::ofstream pseudobulkOut(pseudobulkPath);
    if (!pseudobulkOut) error("Error opening output file: %s for writing", pseudobulkPath.c_str());
    ...
    for (int32_t w = 0; w < M; ++w) {
        ...
        for (int32_t k = 0; k < K; ++k) pseudobulkOut << "\t" << pseudobulk(w, k);
        pseudobulkOut << "\n";
    }
    pseudobulkOut.close();
