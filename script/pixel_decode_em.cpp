...
int32_t cmdPixelDecodeEm(int32_t argc, char** argv) {
    std::string inTsv, inIndex, modelFile, anchorFile, outPref, tmpDirPath, weightFile;
    ...
    ParamList pl;
    pl.add_option("model", "Poisson log1p-NMF model file", modelFile, true)
      ...
      .add_option("pixel-res", "Resolution for pixel-level inference", pixelResolution)
    ...
    pl.add_option("out-pref", "Output prefix", outPref)
      .add_option("output-original", "Output original data points alongside factor results", outputOriginalData)
      .add_option("ext-col-ints", "Additional integer columns to carry over; format \"idx:name\"", annoInts)
      .add_option("ext-col-floats", "Additional float columns to carry over; format \"idx:name\"", annoFloats)
      .add_option("ext-col-strs", "Additional string columns to carry over; format \"idx:name:len\"", annoStrs)
      ...
      .add_option("min-init-count", "Minimum count required to seed an anchor", minInitCount)
      .add_option("output-coord-digits", "Number of decimal digits for coordinates", floatCoordDigits)
      .add_option("output-prob-digits", "Number of decimal digits for probabilities", probDigits)
      .add_option("size-factor", "Size factor used for per-anchor EM updates", sizeFactor)
      ...
      .add_option("max-iter", "Maximum EM iterations per anchor minibatch", maxIter)
      .add_option("mean-change-tol", "Tolerance for EM convergence", mDelta)
    ...
    if (sampleList.empty()) {
        if (outPref.empty() || inTsv.empty() || inIndex.empty())
            error("For single sample analysis --out-pref, --in-tsv, and --in-index must be specified");
    ...
    if (opts.optim.max_iters <= 0 || maxIter <= 0) {
        error("--max-iter-inner and --max-iter-em must be positive");
    }
    ...
    if (sizeFactor <= 0.0) {
        sizeFactor = 1000.0;
        warning("--size-factor must be positive, using default value %.1f", sizeFactor);
    }
    ...
    const int32_t M_model = static_cast<int32_t>(beta.rows());
    const int32_t K_model = static_cast<int32_t>(beta.cols());
    notice("NMF with %d features and %d factors", M_model, K_model);

    // Parser and feature handling
    ...
    EMPoisReg emPois(opts, sizeFactor, exactMLE, debug);
    emPois.init_global_parameter(beta);
    emPois.set_em_options(maxIter, mDelta);

    // Process each dataset
    auto configure_decoder = [&](auto& decoder, const std::string& anchorFile) {
        decoder.setOutputOptions(outputOriginalData, useTicketSystem);
