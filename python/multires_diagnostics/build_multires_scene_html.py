...
.factor-picker-label{color:var(--muted);font-size:12px}.factor-picker-options{display:flex;gap:7px;align-items:center;flex-wrap:wrap}
...
<span id="scene-summary" class="subtitle"></span><span id="navigation" class="nav"></span></div>
<div class="factor-picker"><span class="factor-picker-label">Color right plots by factor abundance</span><span id="scene-factor-picker" class="factor-picker-options"></span></div></section>
...
<section class="card"><div class="title">Scene-restricted global modes · factor abundance</div>
<label class="check-label plot-control"><input id="hide-muted-diffusion-factor" type="checkbox"> Hide muted points</label>
<div id="diffusion-factor-plot" class="plot"></div><div class="hint">Shares the modes and fixed viewport at left. Selected factor theta values are summed; values below 0.05 remain faint.</div></section></div>
<div class="wide diffusion-grid"><section class="card"><div class="title">Supervised next-cluster separation · selected grouping</div><div class="controls">
...
<section class="card"><div class="title">Supervised next-cluster separation · factor abundance</div>
<label class="check-label plot-control"><input id="hide-muted-supervised-factor" type="checkbox"> Hide muted points</label>
<div id="supervised-factor-plot" class="plot"></div><div class="hint">Shares the supervised axes and fixed viewport at left. Uses the scene-wide factor scale selected above.</div></section></div>
<div class="wide diffusion-grid"><section class="card"><div class="title">Quartimax-rotated PCA · selected grouping</div><div class="controls">
...
<section class="card"><div class="title">Quartimax-rotated PCA · factor abundance</div>
<label class="check-label plot-control"><input id="hide-muted-quartimax-pca-factor" type="checkbox"> Hide muted points</label>
<div id="pca-factor-plot" class="plot"></div><div class="hint">Shares the quartimax-PCA axes and fixed viewport at left. Uses the scene-wide factor scale selected above.</div></section></div>
...
const localState={selection:null,source:null,hideMuted:{diffusion_group:false,diffusion_factor:false,supervised:false,supervised_factor:false,quartimax_pca:false,quartimax_pca_factor:false},active:[],groupKey:null,rendering:false,factors:new Set([0])};
const localViewSpecs=[{name:"diffusion",groupPlot:"diffusion-group-plot",factorPlot:"diffusion-factor-plot"},{name:"supervised",groupPlot:"supervised-plot",factorPlot:"supervised-factor-plot"},{name:"quartimax_pca",groupPlot:"pca-plot",factorPlot:"pca-factor-plot"}];
...
function groupVisibilityKey(name){return name==="diffusion"?"diffusion_group":name}
function factorVisibilityKey(name){return`${name}_factor`}
...
function selectedFactorValues(scene){const factors=Array.from(localState.factors),values=Array(scene.members).fill(0);for(let i=0;i<scene.members;i++)for(const factor of factors)values[i]+=scene.theta[i*P.factor_names.length+factor];return values}
function factorUpper(values){return Math.max(.05,quantile(values.filter(value=>value>=.05),.95))}
function factorTraces(name,factorValues,upper){const scene=current(),view=scene.views[name];if(!view.available)return[];const a=currentAxes(view),low=[],high=[],hideMuted=localState.hideMuted[factorVisibilityKey(name)];for(const muted of [true,false])for(const core of [0,1])for(const lowValue of [true,false]){const x=[],y=[],values=[],custom=[];for(let row=0;row<view.rows;row++){const i=view.indices[row],isMuted=localState.selection!==null&&!localState.selection.has(i),value=factorValues[i];if(isMuted!==muted||view.core[row]!==core||(value<.05)!==lowValue)continue;x.push(view.coordinates[row*view.dimensions+a.x]);y.push(view.coordinates[row*view.dimensions+a.y]);values.push(value);custom.push([scene.identifiers[i],i,value,core?"Core":"Halo",scene.membership_score[i],topFactors(scene.theta,i),sceneExternalLabel(scene,i)])}if(!x.length)continue;const trace={type:"scattergl",mode:"markers",x,y,customdata:custom,showlegend:false,visible:muted&&hideMuted?false:true,marker:{size:core?6:5,color:values,colorscale:"Viridis",cmin:0,cmax:upper,showscale:false,opacity:lowValue?(muted?.01:.06):(muted?.025:(core?.72:.20)),symbol:core?"circle":"circle-open"},hoverinfo:muted?"skip":"all",hovertemplate:muted?null:"%{customdata[0]}<br>%{customdata[6]}<br>%{customdata[5]}<br>Selected theta = %{customdata[2]:.4f}<br>%{customdata[3]}; membership score=%{customdata[4]:.3f}<extra></extra>",meta:{muted,lowValue}};(lowValue?low:high).push(trace)}const scale=high.find(trace=>!trace.meta.muted)||high[0];if(scale){scale.marker.showscale=true;scale.marker.colorbar={title:{text:`Theta<br>q95=${fmt(upper)}`},thickness:14,len:.66}}return low.concat(high)}
function drawFactor(name,target,factorValues,upper){const view=current().views[name];if(!view.available)return omitted(target,view.reason);return Plotly.react(target,factorTraces(name,factorValues,upper),viewLayout(view,`${name}-factor`),config)}
async function drawFactorViews(){localState.rendering=true;const scene=current(),values=selectedFactorValues(scene),upper=factorUpper(values);await Promise.all(localViewSpecs.map(spec=>drawFactor(spec.name,spec.factorPlot,values,upper)));localState.rendering=false}
async function drawLocal(){localState.rendering=true;const scene=current(),values=selectedFactorValues(scene),upper=factorUpper(values),jobs=[];for(const spec of localViewSpecs){jobs.push(drawGroupedView(spec.name,spec.groupPlot));jobs.push(drawFactor(spec.name,spec.factorPlot,values,upper))}await Promise.all(jobs);localState.rendering=false;for(const spec of localViewSpecs)if(scene.views[spec.name].available){bindLocal(document.getElementById(spec.groupPlot));bindLocalSelection(document.getElementById(spec.factorPlot));bindViewViewport(spec.groupPlot,spec.factorPlot)}updateLocalSummary()}
...
let viewportSync=false;function bindViewViewport(firstId,secondId){const first=document.getElementById(firstId),second=document.getElementById(secondId);if(first._viewportBound)return;first._viewportBound=second._viewportBound=true;for(const [source,target] of [[first,second],[second,first]])source.on("plotly_relayout",event=>{if(localState.rendering||viewportSync)return;const update={};for(const key of ["xaxis.range[0]","xaxis.range[1]","yaxis.range[0]","yaxis.range[1]","xaxis.autorange","yaxis.autorange"])if(Object.hasOwn(event,key))update[key]=event[key];if(!Object.keys(update).length)return;viewportSync=true;Plotly.relayout(target,update).finally(()=>{viewportSync=false})})}
...
sceneSelect.onchange=drawScene;colorBy.onchange=()=>{localState.groupKey=null;drawLocal()};for(const [id,key] of [["hide-muted-diffusion-group","diffusion_group"],["hide-muted-diffusion-factor","diffusion_factor"],["hide-muted-supervised","supervised"],["hide-muted-supervised-factor","supervised_factor"],["hide-muted-quartimax-pca","quartimax_pca"],["hide-muted-quartimax-pca-factor","quartimax_pca_factor"]])document.getElementById(id).onchange=event=>{localState.hideMuted[key]=event.target.checked;drawLocal()};document.getElementById("clear-local").onclick=()=>{ensureGroups();localState.active.fill(true);setLocalSelection(null,null)};
for(const [id,name,loadingTarget] of [["d-x","diffusion",null],["d-y","diffusion",null],["s-x","supervised","supervised-loading"],["s-y","supervised","supervised-loading"],["p-x","quartimax_pca","pca-loading"],["p-y","quartimax_pca","pca-loading"]])document.getElementById(id).onchange=async()=>{const spec=localViewSpecs.find(value=>value.name===name),scene=current(),values=selectedFactorValues(scene),upper=factorUpper(values);localState.rendering=true;const jobs=[drawGroupedView(name,spec.groupPlot),drawFactor(name,spec.factorPlot,values,upper)];if(loadingTarget)jobs.push(drawLoading(name,loadingTarget));await Promise.all(jobs);localState.rendering=false};
