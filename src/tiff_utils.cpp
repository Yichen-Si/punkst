...
std::vector<uint64_t> read_values(RandomAccessFile& file, const TiffEntry& e, bool le) {
    ...
    for (uint64_t i = 0; i < e.count; ++i) {
        out.push_back(read_scalar_from_bytes(
            data + static_cast<size_t>(i) * step, e.type, le));
    }
    return out;
}

image_tiff::TiffLevel parse_tiff_level(RandomAccessFile& file,
    const image_tiff::TiffInfo& info,
    uint64_t ifdOffset,
...
void validate_tiff_level(const image_tiff::TiffLevel& level,
    const std::filesystem::path& path) {
    if (level.width == 0 || level.height == 0 ||
        level.tileWidth == 0 || level.tileHeight == 0) {
    ...
    if (!(level.compression == 1 || level.compression == 8 ||
            level.compression == 32946)) {
    ...
    if (level.planarConfig != 1) {
        error("%s: only chunky TIFF planar configuration is supported: %s",
            __func__, path.string().c_str());
    }
    ...
    if (rgb) {
        if (!(level.samplesPerPixel == 3 || level.samplesPerPixel == 4) ||
            ...
            error("%s: RGB TIFF must have 3 or 4 samples per pixel: %s",
                __func__, path.string().c_str());
        }
        for (uint16_t b : level.bitsPerSample) {
            if (b != 8) {
                error("%s: RGB TIFF currently requires 8-bit samples: %s",
                    __func__, path.string().c_str());
            }
        }
    } else {
...
bool inflate_exact_with_window_bits(const uint8_t* src, size_t srcSize,
    ...
    if (srcSize > std::numeric_limits<uInt>::max() ||
...
std::vector<uint8_t> zlib_decompress_exact(const uint8_t* src,
    size_t srcSize,
    size_t expectedSize) {
...
image_tiff::TiffInfo image_tiff::parse_tiff_info(RandomAccessFile& file) {
    ...
    const std::vector<uint8_t> header =
        file.read(0, std::min<uint64_t>(16, file.size()));
    TiffInfo info;
    ...
    if (magic == 42) {
        ...
        if (offsetSize != 8) {
            error("%s: unsupported BigTIFF offset size %u",
                __func__, static_cast<unsigned>(offsetSize));
        }
        ifdOffset = read_u64(header, 8, info.littleEndian);
    } else {
        error("%s: unsupported TIFF magic %u in %s", __func__,
            static_cast<unsigned>(magic), file.path().string().c_str());
    }
    std::vector<uint64_t> subIfds;
...
std::vector<uint8_t> image_tiff::decode_tiff_tile(RandomAccessFile& file,
    ...
    if (off > file.size() || n > file.size() - off) {
        error("%s: TIFF tile points outside file: %s",
...
image_tiff::Gray16Bounds image_tiff::estimate_gray16_bounds(
    ...
    const size_t fractionTiles = static_cast<size_t>(
        std::ceil(std::clamp(sampleFraction, 0.0, 1.0) *
    ...
    if (!scanAllTiles && candidateTiles > targetTiles) {
        ...
        for (size_t order = 0; order < indices.size(); ++order) {
            const std::vector<uint8_t> tile =
                decode_tiff_tile(file, level, indices[order], tileBytes);
            size_t nonZero = 0;
            ...
            scores.push_back(TileScore{
                ...
                pixelsPerTile == 0
                    ? 0.0
                    : static_cast<double>(nonZero) / static_cast<double>(pixelsPerTile),
                order});
        }
        const bool hasDenseTile =
            std::any_of(scores.begin(), scores.end(), [](const TileScore& score) {
                return score.density > 0.0;
            });
        if (hasDenseTile) {
            std::stable_sort(scores.begin(), scores.end(),
                [](const TileScore& a, const TileScore& b) {
    ...
    notice("%s: 16-bit grayscale TIFF display bounds from %zu tile sample(s): %u..%u",
        __func__, indices.size(), static_cast<unsigned>(bounds.low),
        static_cast<unsigned>(bounds.high));
    return bounds;
}
