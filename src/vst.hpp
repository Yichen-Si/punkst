...
inline int32_t HVF_VST(const std::vector<Document>& docs, uint32_t M,
    double loess_span = 0.30, double clip_max = -1.0,
    VSTOutputs* stats = nullptr)
{
    ...
    // ---- Pass 1: accumulate sums per feature with thread-local buffers ----
    struct Accum {
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, N, 64),
        ...
        // reuse 'nnz' array in local as scratch? safer to leave it unused here
        for (size_t i = r.begin(); i < r.end(); ++i) {
            const auto& d = docs[i];
    ...
    // First, collect references to thread-local buffers
...
SelectVST(const std::vector<Document>& docs,
              uint32_t n_features, size_t top_k = 0,
              double loess_span = 0.30,
              double clip_max   = -1.0,
              VSTOutputs* dbg   = nullptr)
{
    VSTOutputs out;
    int32_t ret = HVF_VST(docs, n_features, loess_span, clip_max, &out);
    if (dbg) *dbg = out;
    ...
    std::stable_sort(idx.begin(), idx.end(),
        [&](uint32_t a, uint32_t b){
            if (out.var_standardized[a] != out.var_standardized[b])
                return out.var_standardized[a] > out.var_standardized[b];
            return a < b;
        });
    if (top_k > 0 && top_k < idx.size()) idx.resize(top_k);
    return idx;
}
