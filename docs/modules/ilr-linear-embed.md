# ILR Linear Embedding

`punkst ilr-linear-embed` constructs supervised linear views of dense
Gamma-Poisson or LDA topic proportions using one or more hard cluster
partitions. It writes both the between-cluster mean view and the full UAC view
that also captures differences among cluster covariance matrices.

## Usage

```bash
punkst ilr-linear-embed \
...
Theta is floored by `--center-floor` (default `1e-12`), normalized, and mapped
to ILR coordinates. Hard-partition cluster weights, means, and MLE covariance
matrices define
...
positive numerical rank.

`--whitening mixture` is the default and uses
`Sigma = barSigma + M_I`, which is available from the fitted partition
moments. `--whitening sample` uses every theta unit's empirical ILR covariance
centered on the fitted partition mean. Sample whitening adapts axes to the
full cohort but requires an additional quadratic covariance calculation.
...
- `{prefix}.transform.tsv`: long-form raw ILR projections, topic contrasts,
  eigenvalues, contrast scales, signs, and normalized weights for both views.
- `{prefix}.mean.axes.tsv` and `{prefix}.full.axes.tsv`: wide interpretable
  topic-weight matrices. Columns are `w1_p`, `w1_n`, `w2_p`, `w2_n`, and so
  on; positive and negative weights for each axis are separately normalized.
- `{prefix}.results.tsv`: `mean_*` and `full_*` coordinates for every theta
  unit, in theta input order.
