...
- Basic inspection, conversion, and region query
    ...
    - [Export as PMTiles](#write-pmtiles)
...
- supported:
    ...
    - `--prob-dot`
    - `--confusion`
    - direct `--write-mlt-pmtiles` and `--write-mvt-pmtiles`
...
- direct `--write-mlt-pmtiles` and `--write-mvt-pmtiles` also use the embedded dictionary
...
- `--prob-dot`
- `--confusion`
- `--write-mlt-pmtiles`, `--write-mvt-pmtiles`
- `--extract-region`, `--extract-region-geojson`
- `--smooth-top-labels`
- `--spatial-metrics`
- `--shell-surface`
- `--profile-one-factor-mask`
...
- PMTiles metadata records the total factor count `K`; multi-model PMTiles record `K` for each model when available
- if direct PMTiles export reads an already merged binary with multiple factor groups, provide `--Ks` with the total factor count for each group
- `--emb-prefix` can name exported `K/P` column groups, making them selectable later by `--pmtiles-factor-prefix`
...
When PMTiles are written in the same command as `--merge-emb`, per-model factor counts are read from the source inference files automatically. When writing PMTiles later from an already merged binary, provide `--Ks K1 K2 ...` with one total factor count per group. Use `--emb-prefix` when the PMTiles should be selectable later by model id.
...
`--emb-prefix` - (Optional) Rename merged `(K, P)` columns per source. For example, `--k2keep 2 2 --emb-prefix rna atac` writes `rna_K1 rna_P1 rna_K2 rna_P2 atac_K1 atac_P1 atac_K2 atac_P2`.
