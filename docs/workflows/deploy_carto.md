...
The deployment contains transcript PMTiles, gene-bin PMTiles, merged raw-pixel factor annotations, hexagon factor PMTiles, pixel-raster factor PMTiles, the SGE mono basemap density map, sidecar tables, `ficture_assets.json`, and `catalog.yaml`. It does not package imaging, UMAPs, cells, or boundaries.
