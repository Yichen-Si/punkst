...
See [Quickstart](index.md) for an example using the small dataset distributed as `punkst/examples/data/example_data.tar.gz`.
...
punkst topic-model --in-data ${path}/hex_12.txt \
  ...
  --out-prefix ${path}/h12.k12 --transform \
  --threads ${threads} --seed 1
```

The template Makefile uses `h<H>.k<K>` as the default model prefix. For example, `H=12` and `K=12` writes `h12.k12.model.tsv`, `h12.k12.results.tsv`, and downstream files such as `h12.k12.pixel.bin`. The intermediate hexagon count table is still named `hex_12.txt`.
...
```bash
punkst pixel-decode --model ${path}/h12.k12.model.tsv \
  ...
  --out-pref ${path}/h12.k12.pixel --output-binary \
  --temp-dir ${tmpdir} \
  --threads ${threads} --seed 1
```
...
`--output-binary`: Write the main pixel-level output as `${path}/h12.k12.pixel.bin` plus `${path}/h12.k12.pixel.index`. This is the preferred output mode for downstream `tile-op`, visualization, and spatial tests.
...
python punkst/ext/py/color_helper.py \
  --input ${path}/h12.k12.results.tsv \
  --output ${path}/h12.k12.color \
...
```bash
punkst draw-pixel-factors --in ${path}/h12.k12.pixel --binary \
  --in-color ${path}/h12.k12.color.rgb.tsv \
  --out ${path}/h12.k12.pixel.png \
  --scale 1
```
...
```bash
punkst de-chisq \
  --input ${path}/h12.k12.pixel.pseudobulk.tsv \
  --out ${path}/h12.k12.pixel.de_bulk.tsv \
  --threads ${threads}
```
...
python punkst/ext/py/factor_report.py \
  --de ${path}/h12.k12.pixel.de_bulk.tsv \
  --pseudobulk ${path}/h12.k12.pixel.pseudobulk.tsv \
  --feature_label Feature \
  --color_table ${path}/h12.k12.color.rgb.tsv \
  --output_pref ${path}/h12.k12.pixel
```

Optional: add `--de_neighbor ${path}/h12.k12.pixel.de_bulk.1vsNeighbors.tsv` if you included `--neighbor-k` when running `punkst de-chisq` to display top highly specific genes that are enriched even when comparing with the k most similar factors.
