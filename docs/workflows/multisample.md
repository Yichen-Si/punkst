...
### Pixel-level decoding for multiple samples

After fitting a joint model with `topic-model`, run `pixel-decode` with the
per-sample file list generated by `multisample-prepare`. Use `--output-binary`
so each sample is written as binary pixel tiles plus an index for downstream
visualization and spatial analysis.

```bash
punkst pixel-decode --model ./out/merged.hex_12.model.tsv \
    --sample-list ./out/merged.persample_file_list.tsv \
    --icol-x 0 --icol-y 1 --icol-feature 2 --icol-val 3 \
    --hex-grid-dist 12 --n-moves 2 \
    --pixel-res 0.5 \
    --out-pref ./out/merged.pixel --output-binary \
    --temp-dir ./tmp --threads ${threads} --seed 1
```

The `--sample-list` input is the
`[--out-joint-pref].persample_file_list.tsv` file described above. The
`--out-pref` value is used as the shared output prefix for the multi-sample
decode results; see [`pixel-decode`](../modules/pixel-decode.md) for the exact
multi-sample output naming.
